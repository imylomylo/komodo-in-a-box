...
function rewardsunlock {
  KIABMETHOD="rewardsunlock"
  if ps aux | grep komodod | grep $CHAIN | grep -v grep ; then
    echo "rewards name: $REWARDSNAME ||| rewards tx id: $REWARDSTXID"
    sleep 5
    # curl -s --user $rpcuser:$rpcpassword --data-binary "{\"jsonrpc\": \"1.0\", \"id\": \"curltest\", \"method\": \"$KIABMETHOD\", \"params\": [\"$REWARDSNAME\", \"$REWARDSTXID\", \"$LOCKINGTXID\"]}" -H 'content-type: text/plain;' http://127.0.0.1:$rpcport/ | jq -r '.result.hex' > ~/.kiabresponse
    curl -s --user $rpcuser:$rpcpassword --data-binary "{\"jsonrpc\": \"1.0\", \"id\": \"curltest\", \"method\": \"$KIABMETHOD\", \"params\": [\"$REWARDSNAME\", \"$REWARDSTXID\"]}" -H 'content-type: text/plain;' http://127.0.0.1:$rpcport/ | jq -r '.result.hex' > ~/.kiabresponse
    KIABRESPONSE=`cat ~/.kiabresponse`
    RAWHEX=$KIABRESPONSE
    sendrawtransaction
  else
